...
Vector4 ForwardKinematic::computeLocalQuaternion(Joint* joint)
{
	...
	unsigned char rot_order = joint->rotationOrder;
	...
	switch (rot_order)
	{
		case NONE: return Vector4(0,0,0,1);
		case ZYX: x = pFrame[2], y = pFrame[1], z = pFrame[0]; break;
		case YZX: x = pFrame[2], y = pFrame[0], z = pFrame[1]; break;
		case ZXY: x = pFrame[1], y = pFrame[2], z = pFrame[0]; break;
		case XZY: x = pFrame[0], y = pFrame[2], z = pFrame[1]; break;
		case YXZ: x = pFrame[1], y = pFrame[0], z = pFrame[2]; break;
		case XYZ: x = pFrame[0], y = pFrame[1], z = pFrame[2]; break;
	}
	...
	Vector4 q1 = buildQuaternionRotation(x, 1, 0, 0);
	Vector4 q2 = buildQuaternionRotation(y, 0, 1, 0);
	Vector4 q3 = buildQuaternionRotation(z, 0, 0, 1);

	return quaternionMultiplication(q1, quaternionMultiplication(q2, q3));
}
...
Vector4 ForwardKinematic::computeGlobalQuaternion(Joint* joint, Vector4 localQuat)
{
	...
	return quaternionMultiplication(joint->parent->Globalquat, localQuat);
}
...
Vector4 ForwardKinematic::computeGlobalPosition(Joint* joint)
{
	...
	Vector4 Q_Inverted = Vector4(-joint->parent->Globalquat.x, -joint->parent->Globalquat.y, -joint->parent->Globalquat.z, joint->parent->Globalquat.w);
	Vector4 Q_Fake = Vector4(joint->LocalPos.x, joint->LocalPos.y, joint->LocalPos.z, 0);
	Vector4 Q_Prime = quaternionMultiplication(quaternionMultiplication(joint->parent->Globalquat, Q_Fake), Q_Inverted);
		
	joint->GlobalPos = Q_Prime + joint->parent->GlobalPos;
